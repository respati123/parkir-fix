<template>
    <div class="box">
        ...
        <div class="box-body">
            <form @submit.prevent="addTariff" class="form-horizontal">
...
<script>
    export default {
        ...
        methods: {
            addTariff: function(){
                axios.post('/api/v1/tariff/', {tariff: this.tariff, user: this.$store.state.user.userData.name})
                    .then(response => {
                        if(response.data.status === 'success'){
                            this.notif = true;
                            setTimeout(()=>{
                                this.notif = false;
                                this.tariff = {vehicle: '', classes:'', nominal:''}
                            }, 1500)
                        }
